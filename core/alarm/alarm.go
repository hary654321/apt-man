...
import (
	...
	"zrDispatch/common/notify"
	"zrDispatch/common/notify/dingding"
	"zrDispatch/common/notify/email"
	"zrDispatch/common/notify/slack"
	"zrDispatch/common/notify/telegram"
	"zrDispatch/common/notify/wechat"
...
func JudgeNotify(tasklog *define.Log) {
	taskdata, err := model.GetTaskByID(context.Background(), tasklog.TaskID)
	if err != nil {
		log.Error("get task failed", zap.String("taskid", tasklog.TaskID), zap.Error(err))
		return
	}
