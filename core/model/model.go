...
const (
	// TBUser select cuser
	TBUser string = "user"
	// TBHostgroup select cuser
	TBHostgroup string = "hostgroup"
	// TBTask select task
	TBTask string = "task"
	// TBHost select host
	TBHost string = "host"
	...
	TBNotify string = "notify"
	// TBOperate operate table
	TBOperate string = "operate"
...
func QueryUserRule(ctx context.Context, uid string) (define.Role, error) {
	...
	rolesql := `SELECT role FROM user WHERE id=?`
	...
	slog.Println(slog.DEBUG, "QueryUserRule", role)
	if err != nil {
...
func GetNameID(ctx context.Context, t string) ([]define.KlOption, error) {
	...
	if t == TBHost {
		getsql = `SELECT id,ip,lastUpdateTimeUnix FROM ` + string(t)
	}
	conn, err := db.GetConn(ctx)
	if err != nil {
