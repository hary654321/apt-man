...
func EditTask(pge define.ChangeTask) error {
	...
	res := db.Table("task").Select("TaskType", "ip", "port", "cronExpr", "timeout", "routePolicy", "remark", "probeId", "hostGroupID", "run", "threads").Model(&define.ChangeTask{}).Where("id = ?", pge.ID).Updates(pge)
